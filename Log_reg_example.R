...
claimants<-read.csv("C:/Users/Sudheesh Nandakumar/Desktop/Data Science/R_for_Data_Science/Data/claimants.csv")
#Finding null values
sum(is.na(claimants))
...
logit<-glm(ATTORNEY ~ factor(CLMSEX) + factor(CLMINSUR) + factor(SEATBELT) 
           + CLMAGE + LOSS,family= "binomial",data=claimants)  #we convert to factor
summary(logit)
